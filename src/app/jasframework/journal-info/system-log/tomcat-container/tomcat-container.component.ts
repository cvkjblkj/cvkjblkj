...
import { tomcatData } from './../../shared/data';
import { Router, ActivatedRoute, Params } from '@angular/router';
@Component({
  selector: 'tomcat-container',
  templateUrl: 'tomcat-container.component.html',
  styleUrls: ['./tomcat-container.component.css', './../system-log.component.scss', './../../journal-info.component.scss', './../../business-log/business-log.component.scss'],
})
export class TomcatContainerComponent implements OnInit, OnChanges {
  ...
  // public Property: boolean = false;//提示框状态
  constructor(
  ...
  public initLoginListData() {
    ...
    this.commonRequestService.getAppNameList(params, __this, function (res) {
      if (res.success == 1) {
        ...
        let url = '/log/system/tomcat/v2/queryList';
        let params = {
        ...
        __this.journalInfoService.searchData(__this, url, params, function (res) {
          if (res && res.code == 200 && res.success == 1) {
            ...
            for (var i = 0; i < __this.selectListData.length; i++) {
              ...
              __this.selectListData[i].time = __this.selectListData[i]['@timestamp'].replace(/T/, ' ').substring(0, 19);
            };
 ...
 */
  ...
  public changeSelectData(e, i) {
    ...
    if (e.value.name == 'appCode') {
      this.filterSearchData[i][2] = 'appId';
    } else if (e.value.name == 'level') {
      this.filterSearchData[i][2] = 'level';
      this.dropDownList(this.filterSearchData[i][2])
    } else if (e.value.name == 'container_name') {
  ...
  public dropDownList(obj) {
    // this.dropdownData.level = [];
    // this.dropdownData.container_name = [];
    this.dropdownData.appCode = []
    let __this = this;
    if (obj) {
      let url = '/log/system/tomcat/v2/getValues';
      let params = {
        'fields': obj,
        'pageSize': __this.pageSize,
        'pageNum': __this.pageNum,
      ...
      __this.journalInfoService.searchData(__this, url, params, function (res) {
        if (res && res.code == 200 && res.success == 1) {
          if (obj == 'level') {
            __this.arrChange(res['rows'][0].level);
            __this.dropdownData.level = res['rows'][0].level;
          };
 ...
 */
  ...
  downloadShow() {
    ...
    let downloadItems = "{ 'container_name': '容器名称', 'appCode': '应用编码', 'appId': '应用ID', 'level': '日志级别', '@timestamp': '创建时间', 'log': '日志信息' }";
    ...
    let url = '/log/system/tomcat/v2/download';
