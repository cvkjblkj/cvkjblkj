...
import { dockerServiceFileData, loginRankData, dockerRankData } from './../../shared/data';
import { Router, ActivatedRoute } from '@angular/router';
@Component({
  selector: 'docker-service',
  templateUrl: 'docker-service.component.html',
  styleUrls: ['docker-service.component.css', './../system-log.component.scss', './../../journal-info.component.scss', './../../business-log/business-log.component.scss'],
})
export class DockerServiceComponent implements OnInit {
  ...
  constructor(
    ...
    Object.assign(this, { dockerServiceFileData });
    this.loop = [0, 1, 2, 3, 4];
  ...
  public initLoginListData() {
    ...
    this.commonRequestService.getAppNameList(params, __this, function (res) {
      if (res.success == 1) {
        ...
        let url = '/log/system/docker/v2/queryList';
        let params = {
 ...
 */
  public dropdownListData(dropdownData?: any) {
    // this.dropdownData.host = [];
    // this.dropdownData.ident = [];
    //日志级别的筛选数据
    let __this = this;
    if (dropdownData) {

      let url = '/log/system/docker/v2/getValues';
      let params = {
        'fields': dropdownData,
        'pageSize': __this.pageSize,
        'pageNum': __this.pageNum,
 ...
 */
  ...
  downloadShow() {
    ...
    let params = {
      ...
      'token': token,
      // "indexName": "paas-syslog-docker",
      // "indexType": "syslog",
      // "logName": "Docker引擎"
      'pageNum': -1
    }
    let url = '/log/system/docker/v2/download';
