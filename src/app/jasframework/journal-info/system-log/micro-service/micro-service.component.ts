...
import { microServiceFilterData, loginRankData } from './../../shared/data';
...
export class MicroServiceComponent implements OnInit {
  ...
  constructor(
    ...
    Object.assign(this, { microServiceFilterData });
    this.loop = [0, 1, 2, 3, 4];
  ...
  public initLoginListData() {
    ...
    this.commonRequestService.getAppNameList(params, __this, function (res) {
      if (res.success == 1) {
        ...
        let url = '/log/microservice/v2/queryList';
        let params = {
 ...
 */
  ...
  public changeSelectData(e, i) {
    ...
    } else if (e.value.name == 'logLevel') {
      this.filterSearchData[i][2] = 'logLevel'
    } else {
      //如果没有下拉选项，清除数组的最后一项
  ...
  public dropdownListData(appList?: any) {
    //日志级别的筛选数据
    this.dropdownData.logLevel = loginRankData;
 ...
 */
  ...
  downloadShow() {
    ...
    let downloadItems = "{'serviceName': '服务名称','logLevel': '日志级别','linkNode': '链路节点','createTime': '创建时间','errorCode': '异常编码','appName': '应用名称', 'appVersion': '应用版本','appId': '应用id','serviceIp': '服务端IP','servicePort': '服务端端口','requestURI': '请求URI','clientType': '客户端类型', 'userName': '用户姓名','userId': '用户ID','logInfo':'日志信息'}";
    let params = {
    ...
    ///log/system/microservice/v2/download
    let url = '/log/microservice/v2/download ';
